...
| R1 补充：代理节点 = 出口、服务器入出站维护 | 末跳标注“出口（代理节点）”，节点列表前置终点服务器／协议／出口名；服务器详情入口／出口分区，可折叠并跳转引用链；草稿和旧发布按链去重聚合 | 单跳兼入口出口、已有入口复用为末跳；所有入口含未引用项可查看协议／端口／SNI／用途／引用数；出口只统计末跳并展示按链出站配置；限定合成回归及 build/lint 后提交推送，停在 R1 等用户确认 |
