...
## 4. 强交互：从服务器到可用代理的完整闭环

### 4.1 页面信息架构

主导航：**总览｜服务器节点｜代理节点｜拓扑编排｜用户与授权｜订阅与模板｜流量与通知｜设置**。管理员与普通用户仍按现有角色控制；普通用户不看服务器资产和拓扑。
...
1. **进入画布**：并行读取服务器资产、已存在代理和布局。空态只显示“从左侧拖入一台服务器，创建第一个代理节点”，附“添加到画布”按钮作为非拖拽入口。
...
普通路径不要求用户手动点“保存→预览→选择物理根→发布”。高级区仍可查看脱敏配置差异、保存不应用和发布详情，避免以易用为由取消安全步骤。

响应体验必须诚实：当前 `agent/cmd/agent/main.go:33` 的默认心跳／轮询间隔为 30 秒，多阶段发布可能跨越多个轮询周期。UI 提交后立即给出“已受理／等待 Agent”及最后回执时间，不承诺瞬时生效；下一阶段在隔离环境测量后，可增加有退避的独立任务短轮询，而不是为追求动画流畅直接改生产轮询参数。
...
| 场景 | 用户看到的结果 | 后端要求 |
| --- | --- | --- |
| Agent 离线／能力未知 | 可保存草稿；“应用”提示原因 | 第一期不自动排队到未来上线，恢复后重新预检并确认 |
| 端口冲突 | 定位到端口字段及占用代理 | 服务器级事务串行分配；数据库预检不替代 Agent 的实际 bind 检查 |
| 409 版本冲突 | 保留本地表单，展示差异／重新载入 | 禁止最后写入覆盖；使用 draft、graph、server 配置版本向量 |
| HTTP 超时／重复点击 | 查询同一次 operation，不生成两个代理 | 相同幂等键及同一请求体返回同一操作，键被不同请求体复用则 409 |
| prepare 失败 | “校验失败，未应用” | 保留旧期望／确认版本，不晋升订阅 |
| 部分服务器 apply 失败 | 显示成功、失败、回滚中各节点 | 复用补偿回滚；不称跨机器数据库式原子事务；回滚失败为 `manual_required` |
| 撤销移动／未应用字段 | 恢复本地草稿 | 可做本地 undo/redo；不能撤销已经发布的 runtime |
| 回退已发布变更 | “回滚到上次版本”并再次确认 | 新发布动作，校验引用、当前版本和权限；不能直接改数据库指针 |
| 浏览器关闭／刷新 | 已保存草稿可恢复，临时未保存卡有离开提醒 | 草稿落后端；浏览器持久化仅存布局／偏好，不存私钥和配置秘密 |
