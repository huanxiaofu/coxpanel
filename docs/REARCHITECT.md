...
**2026-09-09 用户补充确认（R1）：代理节点 = 出口。** 数据仍是一条 `ProxyDraft.chain`，订阅连接指向首跳，客户端节点名／链名即出口名；最后一个 `ChainHop` 明确显示“出口（代理节点）”。单跳同时是订阅入口与出口；两跳没有中转，三跳才有一个中转。独立 `egress` 仅表示末跳的唯一出网配置，不是另一台服务器或额外一跳。末跳沿用已有入站选择／复用，不复制监听。

`/prototype/proxies` 把出口名称、终点服务器和协议／端口前置，与首跳订阅协议分列。`/prototype/servers` 点击名称或详情打开抽屉，顶部可折叠的入口区列出该机所有资源、协议／端口／SNI／用途、去重链引用及跳转；出口区只列最后一跳位于该机的链、目标／状态和出站 tag／网卡／DNS／路由意图。`getServerInbounds` 与 `getServerEgressRefs` 按服务器从当前工作区聚合，分别标识草稿和旧模拟发布，改草稿不能隐去旧发布引用。当前模型没有独立服务器全局出站设置，摘要按链展示，不虚构生产配置；本轮仍停在 R1 等用户确认。
